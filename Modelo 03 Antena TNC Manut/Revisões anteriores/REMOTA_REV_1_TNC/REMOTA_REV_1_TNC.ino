/*
   REV. 0 ULTIMA ATUALIZACAO 26/03/2019
...
#define delayEmail 300
#define aguardaInicializacao 3000 //Aguarda 2 segundos antes de iniciar o loop
...
String segundo = "";
String horaEmail = "";
String horaStart = "15:17";
String horaEnd = "15:18";
...
void setup() {
  ...
  Serial.begin(9600);
  Serial1.begin(9600, SERIAL_8N1, 4, 2);   // INTERFACE DE COMUNICAÇÃO UART BLUETOOTH
  ...
  if (rtc.lostPower()) {
    ...
    //rtc.adjust(DateTime(2019, 3, 26, 15, 8, 0));
  }

  delay(aguardaInicializacao);

}

void loop() {

  Serial1.print("RECEBENDO,");

  if (!comunicacao) {

    Serial.println("Aguardando sinal da HOST");
    Serial1.print("OLA_HOST,");

    setColorRGB(0, 1, 1); // VERMELHO

    if (Serial1.available()) {

      conversa = Serial1.readStringUntil(',');

      Serial.println(conversa);
  ...
  if (!comunicacao) delay(delayScan);
  if (comunicacao) delay(delayEmail);
