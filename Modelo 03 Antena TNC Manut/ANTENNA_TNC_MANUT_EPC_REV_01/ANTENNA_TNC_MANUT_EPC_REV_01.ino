/*
  REV. 01 ULTIMA ATUALIZACAO 25/05/2019
  PROGRAMADOR E DESENVOLVEDOR: ELIEZER MORAES SILVA
*/

// ******************************************************************************
// ******************* ANTENA LEITORA RFID UHF TECNOLOGIA TNC MANUTENÇÃO *******************
// ******************************************************************************
...
#define COMMON_ANODE
#define SS 53
#define pinEstadoBluetoothCadastrador 7
#define pinEstadoBluetoothEspelho01 6
#define pinEstadoBluetoothEspelho02 5
#define pinReleRESET A5

RFID nano; //Create instance
SoftwareSerial softSerial(11, 10); //TX, RX ARDUINO MEGA 2560 --> RX, TX SHIELD RFID SRTR SPARKFUN
SoftwareSerial antenaEspelho02(12, 13);
File myFile;
DS3231 RTC(SDA, SCL);
...
String conteudoESPELHOpartNumber02 = "";
...
void dataloggerADD();
void listenESPELHO();
void listenESPELHO_02();
void BLOCO_COMPARADOR_CONTADOR_ESPELHO();
void BLOCO_COMPARADOR_CONTADOR_ESPELHO_02();
...
int numeroProduto = 0;
int numeroProdutoCadastrador = 1;
int posicaoRegistro;
...
bool estadoBluetoothCadastrador = false;
bool estadoBluetoothEspelho01 = false;

void setup() {

  pinMode(pinReleRESET, OUTPUT);
  digitalWrite(pinReleRESET, HIGH);

  Serial.begin(9600);     // INICIALIZA A COMUNICAÇÃO SERIAL
  Serial3.begin(38400);   // INICIALIZA A COMUNICAÇÃO UART BLUETOOTH HOST_ESPELHO 01
  ...
  antenaEspelho02.begin(38400); //INICIALIZA A COMUNICAÇÃO UART BLUETOOTH HOST_ESPELHO 02
  RTC.begin();
...
void loop() {
  ...
  estadoBluetoothCadastrador = digitalRead(pinEstadoBluetoothCadastrador);
...
void enviarValoresCadastrador() {
  ...
  if (numeroProdutoCadastrador > quantidadeProdutos) numeroProdutoCadastrador = 1;
...
void leituraTAGS() {
  ...
  if (!estadoBluetoothCadastrador) { //SE O CADASTRADOR ESTIVER LIGADO, NÃO EXECUTA O ESTE BLOCO!
    ...
    if (nano.check() == true) { //Check to see if any new data has come in from module
      ...
      if (responseType == RESPONSE_IS_KEEPALIVE) {
        ...
        //Unknown response
        Serial.println("Módulo M6E Nano em falha!");

        digitalWrite(pinReleRESET, LOW);
...
void BLOCO_COMPARADOR_CONTADOR_ESPELHO() {
  ...
  if (SD.exists(conteudoESPELHO + ".txt")) {
    ...
    Serial.println(" - Doesn't Exist. LEITURA ANTENA REPETIDORA 1 MANUTENÇÃO");
...
void BLOCO_COMPARADOR_CONTADOR_ESPELHO_02() {
  ...
  Serial.println(conteudoESPELHO02);

  if (SD.exists(conteudoESPELHO02 + ".txt")) {

    setColorRGB(0, 1, 1); //COR VERMELHO

    valorQuantidadePRODUTO = EEPROM.read(conteudoESPELHOpartNumber02.toInt());
    ...
    EEPROM.write(conteudoESPELHOpartNumber02.toInt(), valorQuantidadePRODUTO);

    Serial.print("PART NUMBER ESPELHO 02: ");
    Serial.println(conteudoESPELHOpartNumber02.toInt());
    Serial.print("QUANTIDADE EM ESTOQUE: ");
    Serial.println(valorQuantidadePRODUTO);
    ...
    SD.remove(conteudoESPELHO02 + ".txt");
    ...
    Serial.println(" - Doesn't Exist. LEITURA ANTENA REPETIDORA 2 MANUTENÇÃO");
...
void atualizacaoHOSTparaREMOTA() {
  ...
  if (numeroProduto < quantidadeProdutos) {
    ...
    Serial2.println(String(numeroProduto) + ',' + String(valorProdutoBLE));
...
void listenESPELHO() {
  ...
  if (!estadoBluetoothCadastrador) { //SE O CADASTRADOR ESTIVER LIGADO, NÃO EXECUTA O ESTE BLOCO!
    ...
    if (Serial3.available()) {
      ...
      conteudoESPELHOpartNumber = conteudoESPELHO.substring(0, 3);
...
void listenESPELHO_02() {
  ...
  if (!estadoBluetoothCadastrador) { //SE O CADASTRADOR ESTIVER LIGADO, NÃO EXECUTA O ESTE BLOCO!
    ...
    if (antenaEspelho02.available()) {

      conteudoESPELHO02 = antenaEspelho02.readStringUntil(',');

      conteudoESPELHOpartNumber02 = conteudoESPELHO02.substring(0, 3);

      setColorRGB(1, 1, 0); //COR AZUL

      BLOCO_COMPARADOR_CONTADOR_ESPELHO_02();
...
void backupDiariodoEstoque() {
  ...
  //******* 17:00:00 horas ******* Horário para o sistema efetuar o backup dos dados de estoque e salvar uma cópia diariamente no cartão microSD.
  ...
  if (myFile) {
    ...
    for (int d = 1; d <= quantidadeProdutos; d++) {
