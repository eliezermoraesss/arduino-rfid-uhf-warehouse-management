/*
   REV. 2 ULTIMA ATUALIZACAO 26/10/2018
...
void menuSelecao() {
      ...
      while (controleQuantidadeCadastradas > 0 && !comparadorEPC) {
        ...
        if (comparadorEPC && !cancelaLeituraTag) {
          ...
          if (erroEscritaEPC) {
            ...
            SD.remove(String(conteudo) + ".txt");
            
            Serial.println(conteudo);
            Serial.println("Arquivo TID.txt removido com sucesso!");
  ...
  if (n == 1 && !cancelaLeituraTag) {
    ...
    estadoBluetoothPareadoHOST = true; //digitalRead(pinEstadoBluetoothHost);
