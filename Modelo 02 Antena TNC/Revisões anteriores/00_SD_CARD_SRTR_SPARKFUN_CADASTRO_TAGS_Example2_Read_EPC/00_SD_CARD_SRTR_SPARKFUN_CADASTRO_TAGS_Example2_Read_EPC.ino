/*
   REV. 04 ULTIMA ATUALIZACAO 03/10/2018
...
bool estadoBluetoothPareado = true;
...
void setup() {
  ...
  pinMode(pinEstadoBluetooth, INPUT_PULLUP);
...
void botaoOKselecao() {
  ...
  if (estadoAtualBotaoOK && !estadoAnteriorBotaoOK) {
    if (numeroPosicao > 0) {
      n++;
      lcd.clear();
...
void botaoCancelar() {
  ...
  if (estadoAtualBotaoCancelar && !estadoAnteriorBotaoCancelar) {
...
void dataloggerInventario() {
  ...
  estadoBluetoothPareado = digitalRead(pinEstadoBluetooth);

  //Serial.print("ESTADO PAREAMENTO BLUETOOTH: ");
  //Serial.println(estadoBluetoothPareado);

  if (inventarioAtivado && !estadoBluetoothPareado) {
